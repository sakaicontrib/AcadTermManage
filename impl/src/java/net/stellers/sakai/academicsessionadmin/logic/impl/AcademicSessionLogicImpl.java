...
import net.stellers.sakai.academicsessionadmin.dao.AcademicSessionDao;
...
public class AcademicSessionLogicImpl implements AcademicSessionLogic {
	...
	private HashSet<String> getCurrentSessionEIDsFromCache() {
		...
		if (element != null) {
			...
			return (HashSet<String>) element.getObjectValue();
		}
		else {
			return null;
		}
	...
	public List<Semester> getSemesters() {
		...
		if (element != null) {
			...
			return (List<Semester>) element.getObjectValue();
		}
	...
	private void updateCurrentStatus(Semester sem) {
		...
		boolean oldCurrentStatus = currents.contains(sem.getEid());
		...
		synchronized(LOCK_FOR_SITE_UPDATES) {
			...
			if (count > 0) {
				for (Site s : sites) {
					...
					String eid = props.getProperty(SakaiConstants.PROP_NAME_TERM_EID);
					log.debug("site's term eid: " + eid);
					if (oldEID.equals(eid)) {	
						log.debug("updating site properties for "+siteID);
						props.addProperty(SakaiConstants.PROP_NAME_TERM_EID, newEID);
						props.addProperty(SakaiConstants.PROP_NAME_TERM_TITLE, newTitle);
