...
import net.stellers.sakai.academicsessionadmin.tool.Constants;
import net.stellers.sakai.academicsessionadmin.tool.util.ComparatorFactory;
import net.stellers.sakai.academicsessionadmin.tool.util.ReversableComparator;
import net.stellers.sakai.academicsessionadmin.tool.wicketstuff.ActionLink;
import net.stellers.sakai.academicsessionadmin.tool.wicketstuff.ActionPanel;
...
public class SemesterPage extends BasePage implements Constants{
	...
	private static class PagingForm extends Form<Long> {
        ...
        public void onSubmit(){
			...
			// if there's still a feedback message when this form gets submitted, 
			// the feedback box is only half visible => we'll just remove it completely..
	...
	private class SemesterForm extends Form <Semester>{
        ...
        public void onSubmit(){
			...
			log.debug("submit: "+s+"; updateEID:"+updateEID);			
		...
		private void doUpdate(String oldEID, Semester s){
			log.debug("doUpdate()");			
			Semester old = semesterLogic.getSemester(oldEID);
			log.debug("same?"+(old==s));
			try {
	...
	private class SortableSemesterDataProvider extends SortableDataProvider<Semester, String> implements SortingChangeObserver{
		...
		public Iterator<? extends Semester> iterator(long first, long count) {
			...
			log.debug("iterator called: "+first+" / "+count);
